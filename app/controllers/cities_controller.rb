class CitiesController < ApplicationController
  ...
  def show
    ...
    @stamps = @search.result.order("created_at ASC").paginate(:page => params[:page], :per_page => 10)
   ...
   #    @stamps = Stamp.all.where(city_id: @city.id, neighborhood_id: params[:neighborhood_id]).order("created_at ASC").paginate(:page => params[:page], :per_page => 10)
   #  else
   #    @stamps = Stamp.all.where(city_id: @city.id).order("created_at ASC").paginate(:page => params[:page], :per_page => 10)
   #  end
